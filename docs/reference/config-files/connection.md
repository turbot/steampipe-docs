...
| `option` | none     | An optional `option` block to set [connection options](reference/config-files/connection#connection-options) for this connection.   Only `connection` options are supported
| | | |  Additional options are defined in each plugin - refer to the documentation for your plugin on the [Steampipe Hub](https://hub.steampipe.io/plugins)
