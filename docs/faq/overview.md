...
If the plugin lacks a table you need, please visit https://github.com/turbot/steampipe-plugin-x/issues and file a feature request for a new table.
...
To set a memory limit for the Steampipe process, use the `STEAMPIPE_MEMORY_MAX_MB` environment variable. For example, to set a 2GB limit:
...
Alternatively, you can set a default memory limit for all plugins using the `STEAMPIPE_PLUGIN_MEMORY_MAX_MB` environment variable. For example, to set a 2GB limit:
...
Yes. Steampipe exposes a Postgres endpoint that any Postgres client can connect to. Find the connection by starting Steampipe as a service:
...
Some tables require quals, or qualifiers, in the WHERE or JOIN..ON clause of queries. For example, you can't just do this:

```
select * from github_issue
```

Steampipe can't query all of GitHub, you have to scope (qualify) the query, for example:
